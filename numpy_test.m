function resulting = eeglabProcessingcopy(nparray, output_file_path)

%     input_file_path = 'C:\Users\EchoLab\Desktop\Necromancer\temp\1.csv'; % folder to which metrics are exported
%     output_file_path = 'C:\Users\EchoLab\Desktop\Necromancer\temp\computed.csv';
    % get a output_file_path and input_file_path
    % Power (total and by channel) for each frequency band
    % Delta (1-4Hz)
    %This should ensure that the matlab processes dont print anything to the console and hence dont lock it.
    eeg_options( 'debug', 'off' );
    eeg_options( 'verbose', 'off' );
    ...
    dataset = double(nparray);
%     dataset = readmatrix(input_file_path);
    dataset = dataset(:,1:14); % drop columns that aren't frequency bands
    dataset = transpose(dataset);
    assignin('base','asdf',dataset); % because it has a bug where further functions can't read from this function's variables so we made it global.
