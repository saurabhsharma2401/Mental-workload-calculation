...
class Cortex():
	...
	async def query_headset(self,ws):
		QUERY_HEADSET_ID = 2
		...
		await ws.send(json.dumps(query_headset_request))
	...
	async def connect_headset(self,ws):
		...
		await ws.send(json.dumps(connect_headset_request))
		result = await ws.recv()
		result_dic = json.loads(result)
		# print('connect headset result', json.dumps(result_dic, indent=4))


	async def request_access(self,ws):
		REQUEST_ACCESS_ID = 1
		...
		await ws.send(json.dumps(request_access_request))
		result = await ws.recv()
		result_dic = json.loads(result)
	...
	async def authorize(self):
		...
		async with websockets.connect(self.url) as ws:
			await ws.send(json.dumps(authorize_request))
			result = await ws.recv()
			result_dic = json.loads(result)
			if "result" not in result_dic:
				return False
			# print("Authorization result is ",result_dic)
	...
	async def create_session(self, auth, headset_id, ws):
		CREATE_SESSION_ID = 5
		...
		await ws.send(json.dumps(create_session_request))
		result = await ws.recv()
		result_dic = json.loads(result)
		# print("----------------------------")
		print("Create session result is ",result_dic)
	...
	async def grant_access_and_session_info_withoutauthorization(self):
		# ssl_context = ssl.SSLContext(ssl.CERT_NONE)
		async with websockets.connect(self.url) as ws:
			if not await self.query_headset(ws):
				return False
			await self.connect_headset(ws)
			await self.request_access(ws)
			#read self.auth from file here.
			with open("authkey.txt", "r") as keyfile:
				self.auth = keyfile.read()
			keyfile.close()
			if not await self.create_session(self.auth, self.headset_id, ws):
				# print("hiiiii")
				return False
			return True
	...
	async def subRequest(self, ws):
		# print("entered subrequest")
		# print("A NEW SET OF RECORDS ARE ACCESSED HERE ------",time.time())
		SUB_REQUEST_ID = 6 
		subRequest = { 
			"jsonrpc": "2.0", 
			"method": "subscribe", 
		...
		# await ws.send(json.dumps({"id":1, "jsonrpc":"2.0", "method":"querySessions", "params":{"cortexToken":self.auth}}))
		# print(await ws.recv())
		# print("connection created for subrequest")
		...
		for i in range(self.user['number_row_data']):
			
			new_data = await ws.recv()
			
			new_dic = json.loads(new_data)
			if 'eeg' in new_dic:
				# print("received data ", i)
				# print(i, end=' ')
				# print(new_dic['eeg'][2:16])
				data_save.append(new_dic['eeg'][2:16])
				time_unix.append(time.time())
		# print("exiting subrequest")
