...
class G3App(App, ScreenManager):
    ...
    async def backend_discovery(self) -> None:
        ...
        async with G3ServiceDiscovery.listen() as service_listener:
            while True:
                print("~~~~~~~~~~~~~~~~")
                print(await service_listener.events.get())
    ...
    async def handle_control_event(self, g3: Glasses3, event: ControlEventKind) -> None:
        ...
        match event:
            ...
            case ControlEventKind.START_LIVE:
                file = open("first_test.txt", 'a')
                self.start_live_stream(g3,file)
            case ControlEventKind.STOP_LIVE:
                await self.stop_live_stream(g3)
    ...
    def start_live_stream(self, g3: Glasses3, file) -> None:
        async def live_stream():
            ...
            async with g3.stream_rtsp(scene_camera=True, gaze=True) as streams:
                async with streams.scene_camera.decode() as scene_stream, streams.gaze.decode() as gaze_stream:
                    print(type(gaze_stream))
                    #gaze_stream is a asyncio queue wherein we continually get the data.
        ...
        async def update_frame(scene_stream, gaze_stream, streams, file):
            while True:
                ...
                print(self.latest_gaze_with_timestamp[0],time.time())
                file.writelines([str(self.latest_gaze_with_timestamp[0]['gaze2d']), str(time.time())])
                file.flush()
                #HERE WE ARE GETTING THE ACTUAL DATA, RECORD HERE
    ...
    async def stop_live_stream(self,g3) -> None:
        await g3.recorder.start()
